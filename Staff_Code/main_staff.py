...
dataset = input("Which dataset would you like to train your model on? ").lower()
val_resp = False
...
batch_size = 128
epochs = 10
train_history = model.fit(X_test, Y_test, batch_size=batch_size, epochs=epochs, validation_data=(X_test, Y_test))
...
val = False
while val == False:
    resp = input("Would you like to visualize a(nother) filter? ")
    if resp in yes:
